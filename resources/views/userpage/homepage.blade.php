@extends('userpage.master')

@section('homepage')

<div id="homepage">
    <div class="container">
	    <div class="main">
			<div class="row">
				<div class="col-md-8 main_left">
					<div class="slide">
						<img alt="" class="" src="https://zmp3-photo.zadn.vn/banner/9/5/95de47057c51fc093e9763f83c177940_1510310771.jpg">
					</div>
					<div class="row">
						<div class="song">
							<h2>Bai Hat</h2>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten bai hat</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten bai hat</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten bai hat</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten bai hat</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten bai hat</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten bai hat</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten bai hat</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten bai hat</p>
								<p>Ten ca si</p>
							</div>
						</div>
						<div class="album">
							<h2>Album</h2>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten album</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten album</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten album</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten album</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten album</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten album</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten album</p>
								<p>Ten ca si</p>
							</div>
							<div class="col-md-3 product">
								<img src="https://zmp3-photo.zadn.vn/thumb/240_240/covers/c/e/ce4ac876e42f82e8ce29bece5c2f703b_1509074377.jpg" alt="" class="">
								<p>Ten album</p>
								<p>Ten ca si</p>
							</div>
						</div>
					</div>
				</div>
				<div class="col-md-4 main_right">
					<div class="sidebar">
						<img src="https://zmp3-photo.zadn.vn/covers/e/7/e776fa76156c50e4793a82a52b9d5dd6_1499827947.jpg" class="img-responsive" alt="">
						<img src="https://zmp3-photo.zadn.vn/covers/1/e/1e8a106b249567119009a8149e1b77ff_1499827613.jpg" class="img-responsive" alt="">
						<img src="https://zmp3-photo.zadn.vn/covers/4/1/41338edebf610de7a683e3dab33f8b1a_1499827990.jpg" class="img-responsive" alt="">
					</div>
					<div class="Hotlist">
						<div class="hit_song">
							<h2>BXH bai hat</h2>
							<div class="group">
								<span>01</span>
								<h3><a href="">Ten Bai Hat</a></h3>
								<h4><a href="">Ten Ca Si</a></h4>
								<p>vote</p>
							</div>
							<div class="group">
								<span>02</span>
								<h3><a href="">Ten Bai Hat</a></h3>
								<h4><a href="">Ten Ca Si</a></h4>
								<p>vote</p>
							</div>
							<div class="group">
								<span>03</span>
								<h3><a href="">Ten Bai Hat</a></h3>
								<h4><a href="">Ten Ca Si</a></h4>
								<p>vote</p>
							</div>
							<div class="group">
								<span>04</span>
								<h3><a href="">Ten Bai Hat</a></h3>
								<h4><a href="">Ten Ca Si</a></h4>
								<p>vote</p>
							</div>
							<div class="group">
								<span>05</span>
								<h3><a href="">Ten Bai Hat</a></h3>
								<h4><a href="">Ten Ca Si</a></h4>
								<p>vote</p>
							</div>
							<div class="group">
								<span>06</span>
								<h3><a href="">Ten Bai Hat</a></h3>
								<h4><a href="">Ten Ca Si</a></h4>
								<p>vote</p>
							</div>
							<div class="group">
								<span>07</span>
								<h3><a href="">Ten Bai Hat</a></h3>
								<h4><a href="">Ten Ca Si</a></h4>
								<p>vote</p>
							</div>
							<div class="group">
								<span>08</span>
								<h3><a href="">Ten Bai Hat</a></h3>
								<h4><a href="">Ten Ca Si</a></h4>
								<p>vote</p>
							</div>
							<div class="group">
								<span>09</span>
								<h3><a href="">Ten Bai Hat</a></h3>
								<h4><a href="">Ten Ca Si</a></h4>
								<p>vote</p>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="company">
				<img src="https://zmp3-static.zadn.vn/skins/zmp3-v5.1/images/doi-tac/universal.jpeg" alt="Universal">
				<img src="https://zmp3-static.zadn.vn/skins/zmp3-v5.1/images/doi-tac/universal.jpeg" alt="Universal">
				<img src="https://zmp3-static.zadn.vn/skins/zmp3-v5.1/images/doi-tac/universal.jpeg" alt="Universal">
				<img src="https://zmp3-static.zadn.vn/skins/zmp3-v5.1/images/doi-tac/universal.jpeg" alt="Universal">
				<img src="https://zmp3-static.zadn.vn/skins/zmp3-v5.1/images/doi-tac/universal.jpeg" alt="Universal">
				<img src="https://zmp3-static.zadn.vn/skins/zmp3-v5.1/images/doi-tac/universal.jpeg" alt="Universal">
			</div>
		</div>
	</div>
</div>
@endsection
